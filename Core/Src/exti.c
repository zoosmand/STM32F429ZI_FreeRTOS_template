...
void EXTI_Init(void) {
  ...
  SET_BIT(EXTI->IMR, (EXTI_IMR_IM0 | EXTI_IMR_IM1 | EXTI_IMR_IM2 | EXTI_IMR_IM15));     /* set interrupts */
  // CLEAR_BIT(EXTI->FTSR, (EXTI_IMR_IM0 | EXTI_IMR_IM15));  /* clear fallng egde */
  SET_BIT(EXTI->RTSR, EXTI_IMR_IM0 | EXTI_IMR_IM1 | EXTI_IMR_IM2);    /* set rising edge */
  SET_BIT(EXTI->FTSR, EXTI_IMR_IM15);    /* set fallng edge */

  /* Wake-Up PA0, interrupt Init */
  NVIC_SetPriority(EXTI0_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 15, 0));
  NVIC_EnableIRQ(EXTI0_IRQn);

  /* MEMS INT1 PA1, interrupt Init */
  NVIC_SetPriority(EXTI1_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 15, 0));
  NVIC_EnableIRQ(EXTI1_IRQn);

  /* MEMS INT2 PA2, interrupt Init */
  NVIC_SetPriority(EXTI1_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 15, 0));
